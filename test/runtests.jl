...
sequence_files_dir = joinpath(dirname(@__FILE__), "sequence_files")
if !isdir(sequence_files_dir)
	mkdir(sequence_files_dir)
end
open(cfg_file, "w") do f
	...
	cfg["sequence_files_dir"] = sequence_files_dir
	JSON.print(f, cfg)
end
